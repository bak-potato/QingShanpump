...
<div class="dshoptitleta">
  <el-card
    shadow="hover"
    v-for="(item, index) in paginatedApps"
    :key="index"
    style="max-width: 500px; width: 440px;"
  >
    <div class="dshoptitleta2"></div>
    <div class="dshoptitleta3">
      <div class="head"></div>
      <div class="content">
        <h3 class="title">应用名称 {{ (currentPage - 1) * pageSize + index + 1 }}</h3>
        <p class="description">应用介绍</p>
...
const pageSize = 9; // 每页显示9个应用
const totalApps = ref(21); // 总应用数量

// 模拟应用数据
const apps = Array.from({ length: totalApps.value }, (_, i) => ({
  id: i + 1,
  name: `应用 ${i + 1}`,
  description: '应用介绍'
}));
